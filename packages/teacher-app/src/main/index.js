...
function startBackend() {
  const isDev = !!process.env.VITE_DEV_SERVER_URL
  if (isDev) {
    const backendDir = path.join(__dirname, '../../../teacher-backend')
    console.log(`[Dev] 启动教师端后端: python -m app.main (cwd: ${backendDir})`)
    ...
    backendProcess.stdout.on('data', (data) => {
      console.log(`[teacher-backend] ${data.toString().trim()}`)
    })
    backendProcess.stderr.on('data', (data) => {
      console.error(`[teacher-backend] ${data.toString().trim()}`)
    })
    backendProcess.on('error', (err) => {
      console.error('教师端后端启动失败:', err)
    })
    backendProcess.on('exit', (code) => {
      console.log(`教师端后端退出，代码: ${code}`)
      backendProcess = null
    })
    return
  }
...
app.whenReady().then(async () => {
  ...
  if (backendProcess) {
    try {
      ...
      console.log(`[Dev] 教师端后端已就绪: http://127.0.0.1:${LOCAL_API_PORT}`)
    } catch (err) {
